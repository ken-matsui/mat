...
public class CodeGenerator implements mat.sysdep.CodeGenerator,
        MIRVisitor<Void,Void>, ELFConstants {
    // #@@range/ctor{
    ...
    // #@@range/generate{
    public AssemblyCode generate(MIR mir) {
        locateSymbols(mir);
        return generateAssemblyCode(mir);
    }
    // #@@}
    ...
    // #@@range/locateSymbols{
    private void locateSymbols(MIR mir) {
        SymbolTable constSymbols = new SymbolTable(CONST_SYMBOL_BASE);
        for (ConstantEntry ent : mir.constantTable().entries()) {
            locateStringLiteral(ent, constSymbols);
        }
        for (Variable var : mir.allGlobalVariables()) {
            locateGlobalVariable(var);
        }
        for (Function func : mir.allFunctions()) {
            locateFunction(func);
        }
    }
    // #@@}
    ...
    private AssemblyCode generateAssemblyCode(MIR mir) {
        AssemblyCode file = newAssemblyCode();
        file._file(mir.fileName());
        if (mir.isGlobalVariableDefined()) {
            generateDataSection(file, mir.definedGlobalVariables());
        }
        if (mir.isStringLiteralDefined()) {
            generateReadOnlyDataSection(file, mir.constantTable());
        }
        if (mir.isFunctionDefined()) {
            generateTextSection(file, mir.definedFunctions());
        }
        // #@@range/generateAssemblyCode_last{
        if (mir.isCommonSymbolDefined()) {
            generateCommonSymbols(file, mir.definedCommonSymbols());
        }
        if (options.isPositionIndependent()) {
