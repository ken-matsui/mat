...
import mat.exception.JumpError;
import mat.exception.SemanticException;
import mat.mir.*;
import mat.type.Type;
import mat.type.TypeTable;
import mat.utils.ErrorHandler;
import mat.utils.ListUtils;

import java.util.*;

class MIRGenerator implements ASTVisitor<Void, Expr> {
    private final TypeTable typeTable;
    private final ErrorHandler errorHandler;

    // #@@range/ctor{
    public MIRGenerator(TypeTable typeTable, ErrorHandler errorHandler) {
    ...
    // #@@range/generate{
    public MIR generate(AST ast) throws SemanticException {
    ...
    public Void visit(ExprStmtNode node) {
        ...
        if (e != null) {
            ...
            errorHandler.warn("useless expression", node.location());
        }
        return null;
    }
...
//    }
    ...
    private void checkJumpLinks(Map<String, JumpEntry> jumpMap) {
        for (Map.Entry<String, JumpEntry> ent : jumpMap.entrySet()) {
            ...
            if (!jump.isDefined) {
                errorHandler.error("undefined label: " + labelName, jump.location);
            }
            if (jump.numRefered == 0) {
                errorHandler.warn("useless label: " + labelName, jump.location);
            }
        }
    }
...
//    }
    ...
    // #@@range/Funcall{
    public Expr visit(FnCallNode node) {
...
//    }
    ...
    private mat.asm.Type asmType(Type t) {
        if (t.isVoid()) return int_t();
        return mat.asm.Type.get(t.size());
    }

    private mat.asm.Type varType(Type t) {
        if (! t.isScalar()) {
            return null;
        }
        return mat.asm.Type.get(t.size());
    ...
    private mat.asm.Type int_t() {
        return mat.asm.Type.get((int)typeTable.intSize());
    }

    private mat.asm.Type size_t() {
        return mat.asm.Type.get((int)typeTable.longSize());
    }

    private mat.asm.Type ptr_t() {
        return mat.asm.Type.get((int)typeTable.pointerSize());
    }

    private mat.asm.Type ptrdiff_t() {
        return mat.asm.Type.get((int)typeTable.longSize());
    }

    private void error(Node n, String msg) {
        errorHandler.error(msg, n.location());
    }
}
